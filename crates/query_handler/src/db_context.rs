...
pub async fn init_db() -> Result<(), DbErr> {
    ...
    DB_CONNECTION.set(connection).expect("Database connection has already been initialized");
    
    Ok(())
}

pub fn get_db_connection() -> &'static DatabaseConnection {
    DB_CONNECTION.get().expect("Database connection not initialized. Call init_db() first.")
}